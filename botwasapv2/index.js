...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
           ...
           *     await client.groupRemove(groupId, sender)
			...
			if (getLevelingLevel(sender) < ahd ) return reply(ind.reglevelahd(command, pushname, getLevelingLevel, sender, ahd))
				...
				if (getLevelingLevel(sender) < ahb ) return reply(ind.reglevelahb(command, pushname, getLevelingLevel, sender, ahb))
					...
					rano = getRandom('.ogg')
					dtt.length > 300
					? reply('Textnya jangan panjang panjang om😓')
				...
				case 'setprefix':
					...
					prefix = args[0]
					reply(`Prefix sudah di ganti ya tod : ${prefix}`)
					break 
				case 'hilih': 
				if (!isRegistered) return reply(ind.noregis())
				if (getLevelingLevel(sender) < ahc ) return reply(ind.reglevel(command, pushname, getLevelingLevel, sender, ahc))
				...
				case 'clearall':
				 ...
				 client.chatRead (from)
					...
					client.sendMessage(from, `𝗽𝗲𝗿𝗶𝗻𝘁𝗮𝗵 𝗗𝗶𝘁𝗲𝗿𝗶𝗺𝗮, 𝗺𝗲𝗺𝗯𝘂𝗸𝗮 blockir ${body.slice(9)}@c.us`, text)
				break
				case 'leave': 
				if (!isGroup) return reply(ind.groupo())
					if (!isOwner) return reply(ind.ownerb())
				...
				case 'bc': 
			   	...
			   	case 'setpp': 
                       ...
                       media = await client.downloadAndSaveMediaMessage(mek)
                        ...
                        reply('*IKON GROUP SUDAH DIGANTI TOD*')
                break						
				case 'add':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (args.length < 1) return reply('Yang mau di add siapa njer')
					...
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (args[0] === 'buka') {
					    reply(`Group berhasil dibuka Tod`)
						client.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'tutup') {
						reply(`Udah ku tutup Tod, mau apa lagi?`)
       ...
       client.sendMessage(from, 'Tuh nomer owner nya chat aja, Kalo gak di bales spam telpon aja Tapi jangan VC',MessageType.text, { quoted: mek} )
           break    
           case 'setname':
                if (!isGroup) return reply(ind.groupo())
			    if (!isGroupAdmins) return reply(ind.admin())
                ...
                client.sendMessage(from, 'Udah diganti nama group nya tod', text, {quoted: mek})
                break
                case 'setdesc':
                if (!isGroup) return reply(ind.groupo())
			    if (!isGroupAdmins) return reply(ind.admin())
                ...
                client.sendMessage(from, 'Desc Berhasil diganti', text, {quoted: mek})
                break
           case 'demote':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `Mampus kena demote yahaha wahyu :\n`
							teks += `@_.split('@')[0]`
						}
				...
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Selamat anda sekarang adalah admin group :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`Selamat kakak @${mentioned[0].split('@')[0]} anda menjadi admin group(+_+)`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Otw kick yahahaha ngakak kena kick :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`Otw kick yahahaha ngakak kena kick @${mentioned[0].split('@')[0]} 🏃`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
				...
				if (getLevelingLevel(sender) < ahb ) return reply(ind.reglevelahb(command, pushname, getLevelingLevel, sender, ahb))
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ini kak sudah jadi '})
						fs.unlinkSync(ran)
					})
					break
                 case 'simi':
                ...
                if (!isGroup) return reply(ind.groupo())
                if (!isGroupAdmins) return reply(ind.admin())
                if (args.length < 1) return reply('Ngerepotin aja njir')
                if (args[0] === 'enable') {
